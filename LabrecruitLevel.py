...
# ----------------------------------------------------------------------------------------------------------
def getState(G,buttonmap, node_pos,qtable,training):
    state =""
    possibleActionList =[]
    ...
    for i in range(len(G.nodes[str(node_pos)]['VertexLabel'])):
        #check the status of the button
        status =0
        if (G.nodes[str(node_pos)]['VertexLabel'][i] in buttonmap):
            status = buttonmap[G.nodes[str(node_pos)]['VertexLabel'][i]]['status']
        buttonname = str(G.nodes[str(node_pos)]['VertexLabel'][i])  # b means button
        state = state + " " + buttonname + '-' + str(status)
...
def QlearningTraining(G,buttonmap, numTrainingEpisodes, epsilonGreedyAlgo):
    ...
    for episode in range(numTrainingEpisodes):
        ...
        currentState, possibleActionList = getState(G_, buttonmap, current_pos, qtable, True)
        ...
        while (isterminalState(current_pos) == False):
            ...
            nextState, possibleActionList = getState(G_, buttonmap,next_state_pos, qtable, True)
...
if __name__ == '__main__':
    ...
    if (training == True): # run the training session
        ...
        start = time.time()
        Q_table = QlearningTraining(G,buttonMapping,numTrainingEpisodes, epsilonGreedyAlgo)
