...
goalState = 872   # identify the destination room
...
def QlearningTraining(G,numTrainingEpisodes, epsilonGreedyAlgo):
    ...
    for episode in range(numTrainingEpisodes):
        ...
        while (isterminalState(current_pos) == False):
            ...
            if (step > 500):
                print('more than 400 steps, terminating this episode')
                break
...
def checkshortestpathlength(G, initialstate):
    shortestpahtlength=[]
    #allShortestPaths={}
    for startingnode in range(len(initialstate)):
        if (nx.has_path(G,startingnode,goalState)==True):
            shortestpahtlength.append(nx.shortest_path_length(G, source=startingnode, target=goalState))
...
if __name__ == '__main__':
    ...
    graphoutputpath = 'results/disconnectedmaze'#'results/graph'
    ...
    if (training == False): # training has already done, only need to load the trained Q-table
        filename = 'results/Qtable.pickle'
    ...
    if(createtestset == True):
        ...
        while i < numberoftestepisode:
            pos = np.random.randint(0, G.number_of_nodes())
